...
const HeroFissionDiagram = () => {
  ...
  const LayerLine = ({ x, label, delay }: { x: number; label: string; delay: number }) => (
    <g>
      <motion.line
        x1={`${x}%`} y1="20%" x2={`${x}%`} y2="84%"
        stroke="hsl(var(--border))" strokeWidth="1" strokeOpacity={0.5}
  ...
  const Nucleus = ({ cx, cy, r, delay, label, labelY }: { cx: number; cy: number; r: number; delay: number; label: string; labelY?: "above" | "below" }) => {
    const dots = Array.from({ length: Math.floor(r * 0.5) }).map((_, i) => {
      const angle = (i / Math.floor(r * 0.5)) * Math.PI * 2 + (i % 2 === 0 ? 0.4 : 0);
    ...
    return (
      <g>
        <motion.circle
          cx={cx} cy={cy} r={r}
          fill="none"
          stroke="hsl(var(--primary))" strokeWidth="1.2" strokeOpacity={0.4}
          initial={{ scale: 0 }}
        ...
        {dots}
        <motion.text
          x={cx} y={labelY === "below" ? cy + r + 18 : cy - r - 10}
          textAnchor="middle"
          fill="hsl(var(--muted-foreground))" fontSize="12" fontFamily="var(--font-body)"
          initial={{ opacity: 0 }}
          animate={isInView ? { opacity: 0.7 } : {}}
  ...
  const NeutronPath = ({ x1, y1, x2, y2, delay, showDot = true }: { x1: number; y1: number; x2: number; y2: number; delay: number; showDot?: boolean }) => (
    <g>
      <motion.line
        ...
        stroke="hsl(var(--primary))" strokeWidth="1" strokeOpacity={0.35}
        strokeDasharray="4 4"
        initial={{ pathLength: 0 }}
        animate={isInView ? { pathLength: 1 } : {}}
  ...
  // Layout constants (viewBox 0 0 1000 340)
  const midY = 170;

  // Stage positions (x-axis)
  const stageX = { neutron: 50, target: 220, fission: 360, fragments: 500, released: 650, secondary: 840 };

  // Fragment positions
  const frag1 = { cx: stageX.fragments, cy: midY - 65 };
  const frag2 = { cx: stageX.fragments, cy: midY + 65 };

  // Released neutron endpoints
  const released = [
    { x: stageX.released, y: midY - 105 },
  ...
  // Secondary targets
  const secondaries = [
    { cx: stageX.secondary, cy: midY - 105 },
    ...
    <div ref={ref} className="w-full">
      ...
      >
        <div className="relative w-full" style={{ height: "260px" }}>
          <svg width="100%" height="100%" viewBox="0 0 1000 340" preserveAspectRatio="xMidYMid meet">
            <defs>
            ...
            <motion.text
              ...
              fontSize="10" fontWeight="500" fontFamily="var(--font-body)" letterSpacing="2.5"
              initial={{ opacity: 0 }}
            ...
            <LayerLine x={90} label="CHAIN" delay={0.55} />

            {/* ── Stage 1: Incoming neutron ── */}
            <motion.text
              x={stageX.neutron - 20} y={midY - 16}
              fill="hsl(var(--primary))" fontSize="12" fontFamily="var(--font-body)"
              initial={{ opacity: 0 }}
            ...
            <NeutronPath x1={stageX.neutron} y1={midY} x2={stageX.target - 38} y2={midY} delay={0.4} />

            {/* ── Stage 2: U-235 target ── */}
            <Nucleus cx={stageX.target} cy={midY} r={32} delay={0.5} label="U-235" labelY="below" />

            {/* ── Fission flash ── */}
            <motion.circle
              cx={stageX.fission - 20} cy={midY} r={50}
              fill="url(#fissionFlash)"
              initial={{ opacity: 0, scale: 0 }}
              animate={isInView ? { opacity: [0, 0.8, 0], scale: [0.3, 1.3, 1.6] } : {}}
            ...
            {/* ── Energy label ── */}
            <motion.text
              x={stageX.fission - 20} y={midY + 5}
              textAnchor="middle"
              fill="hsl(var(--primary))" fontSize="11" fontFamily="var(--font-body)" fontWeight="500"
              initial={{ opacity: 0 }}
              animate={isInView ? { opacity: [0, 0.8, 0.4, 0.8] } : {}}
            ...
            <NeutronPath x1={stageX.target + 32} y1={midY} x2={frag1.cx - 22} y2={frag1.cy} delay={1.7} showDot={false} />
            <NeutronPath x1={stageX.target + 32} y1={midY} x2={frag2.cx - 20} y2={frag2.cy} delay={1.7} showDot={false} />
            ...
            >
              <Nucleus cx={frag1.cx} cy={frag1.cy} r={22} delay={1.9} label="Ba-141" labelY="above" />
              <Nucleus cx={frag2.cx} cy={frag2.cy} r={19} delay={1.9} label="Kr-92" labelY="below" />
            </motion.g>
            ...
            {released.map((r, i) => (
              <g key={`rel-${i}`}>
                <NeutronPath
                  x1={i === 1 ? frag1.cx + 22 : (i === 0 ? frag1.cx : frag2.cx)}
                  y1={i === 1 ? frag1.cy : (i === 0 ? frag1.cy - 19 : frag2.cy + 19)}
                  x2={r.x} y2={r.y}
                  delay={2.3 + i * 0.1}
                />
                <motion.text
                  x={r.x + 10} y={r.y + 4}
                  fill="hsl(var(--primary))" fontSize="11" fontFamily="var(--font-body)" fontStyle="italic"
                  initial={{ opacity: 0 }}
                  animate={isInView ? { opacity: 0.7 } : {}}
            ...
            {secondaries.map((s, i) => (
              <g key={`sec-${i}`}>
                <NeutronPath x1={released[i].x} y1={released[i].y} x2={s.cx - 24} y2={s.cy} delay={3.0 + i * 0.12} showDot={false} />
                <motion.circle
                ...
                <motion.text
                  x={s.cx} y={s.cy + 5}
                  textAnchor="middle"
                  fill="hsl(var(--primary))" fontSize="14" fontFamily="var(--font-body)"
                  initial={{ opacity: 0 }}
                  animate={isInView ? { opacity: [0, 0.5, 0.25, 0.5] } : {}}
